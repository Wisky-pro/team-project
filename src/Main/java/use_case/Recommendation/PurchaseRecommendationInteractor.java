...
public class PurchaseRecommendationInteractor implements PurchaseRecommendationInputBoundary {
    ...
    public void execute(PurchaseRecommendationInputData inputData) {
        ...
        double sum = getSum(history);
        double mean = sum / history.size();

        double currentPrice = dataAccess.getCurrentPrice(name);

        PurchaseRecommendationOutputData outputData = getRecommendationOutputData(currentPrice, mean, name);
        presenter.prepareSuccessView(outputData);
    }

    private static PurchaseRecommendationOutputData getRecommendationOutputData(double currentPrice, double mean, String name) {
        String suggestion;
    ...
    private static double getSum(List<Double> history) {
        double sum = 0.0;
